...
std::string RFMPC::XmlPath() const {
  return GetModelPath("rfmpc/task_2_dof.xml");
}
std::string RFMPC::Name() const { return "2 DOF RFMPC"; }

void RFMPC::ResidualFn::Residual(const mjModel* model,
                                         const mjData* data,
  ...
  for (RFMPCFoot foot : kFootAll) {
    ...
    // printf("foot #%d, height target: %f\n", foot, height_target);
    double height_difference = foot_pos[foot][2] - height_target;

    // printf("foot #%d, height: %f\n", foot, foot_pos[foot][2]);

    
    // printf("foot #%d, height difference: %f\n", foot, height_difference);
    residual[counter++] = step[foot] ? height_difference : 0;
  }

  // WriteFeetPosition(step);
...
//  ============  transition  ============
void RFMPC::TransitionLocked(mjModel* model, mjData* data) {
  ...
  if (gait_selection != residual_.current_gait_) {
    ...
    ResidualFn::RFMPCGait gait = residual_.GetGait();
  ...
  // save current mode
  residual_.current_mode_ = static_cast<ResidualFn::RFMPCMode>(mode);
...
void RFMPC::ModifyScene(const mjModel* model, const mjData* data,
                           mjvScene* scene) const {
  // printf("ModifyScene\n");

  // current foot positions
  double* foot_pos[ResidualFn::kNumFoot];
  for (ResidualFn::RFMPCFoot foot : ResidualFn::kFootAll) {
...
void RFMPC::ResetLocked(const mjModel* model) {
  // printf("Reset\n");
...
void RFMPC::ResidualFn::AverageFootPos(double avg_foot_pos[3],
...
double RFMPC::ResidualFn::GetPhase(double time) const {
  return phase_start_ + (time - phase_start_time_) * phase_velocity_;
}

// horizontal Walk trajectory
void RFMPC::ResidualFn::Walk(double pos[2], double time) const {
...
// get gait
RFMPC::ResidualFn::RFMPCGait RFMPC::ResidualFn::GetGait() const {
  return static_cast<RFMPCGait>(ReinterpretAsInt(parameters_[gait_param_id_]));
}

// return normalized target step height
double RFMPC::ResidualFn::StepHeight(double time, double footphase,
...
void RFMPC::ResidualFn::FootStep(double target_step_height[kNumFoot], double time,
                             RFMPCGait gait) const {
