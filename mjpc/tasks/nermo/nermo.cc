...
void Nermo::ResidualFn::Residual(const mjModel* model, const mjData* data, double* residual) const {
  ...
  mju_sub(residual + counter, data->qpos + 7, home + 7, model->nq - 7);
  ...
  for (NermoFoot foot : kFootAll) 
  {
    for (int j = 0; j < 2; j++)
    {
      residual[counter + kMotorJointIds[foot][j]] = 0;
    }
...
void Nermo::TransitionLocked(mjModel* model, mjData* data) {
  ...
  if (gait_selection != residual_.current_gait_) {
    ...
    ResidualFn::NermoGait gait = residual_.GetGait();
  ...
  // save current mode
  residual_.current_mode_ = static_cast<ResidualFn::NermoMode>(mode);
...
void Nermo::ResetLocked(const mjModel* model) {
  ...
  // ------------ model identifiers ---------------
  residual_.torso_body_id_ = mj_name2id(model, mjOBJ_XBODY, "mouse");
  if (residual_.torso_body_id_ < 0) mju_error("body 'mouse' not found");

  residual_.head_site_id_ = mj_name2id(model, mjOBJ_SITE, "head_site");
  if (residual_.head_site_id_ < 0) mju_error("site 'head_site' not found");
  ...
  // foot geom ids
  int foot_index = 0;
  for (const char* footname : {"foot_placement_fl2", "foot_placement_rl2", 
                              "foot_placement_fr2", "foot_placement_rr2"}) {
...
void Nermo::ModifyScene(const mjModel* model, const mjData* data, mjvScene* scene) const {
  ...
  for (ResidualFn::NermoFoot foot : ResidualFn::kFootAll) {
  ...
  // ground under CoM
  // floor is in group 2
  // const mjtByte geomgroup[6] = {0, 0, 1, 0, 0, 0};

  //   for (int i = 0; i < 6; i++)
  // {
  //   printf("query_geomgroup[%d] = %d\n", i, geomgroup[i]);
  // }
...
Nermo::ResidualFn::NermoGait Nermo::ResidualFn::GetGait() const {
  return static_cast<NermoGait>(ReinterpretAsInt(current_gait_));
}
